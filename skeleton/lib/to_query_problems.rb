...
def price_like_shiny_mouse
  ...
  execute(<<-SQL) 
    SELECT
      toys_b.name, toys_a.price
    FROM
      toys AS toys_a
    JOIN
      toys AS toys_b ON toys_b.price = toys_a.price
    WHERE
      toys_a.name = 'Shiny Mouse' AND toys_b.name != 'Shiny Mouse'
...
def just_like_orange
  ...
  execute(<<-SQL)
  SELECT
    a_breed.name, b.breed
  FROM
    cats AS a_breed
  JOIN 
    cats as b ON a_breed.breed = b.breed
  WHERE
    b.name = 'Orange' AND a_breed.name != 'Orange'
  ORDER BY
    a_breed.name ASC
...
def toys_that_jet_owns
  ...
  execute(<<-SQL)
  SELECT 
    DISTINCT(cats.name), toys.name AS toys
  FROM 
    cats
  JOIN 
  ...
  JOIN
    toys ON toys.id = cattoys.toy_id
  JOIN
    cattoys AS cattoy_test ON cattoy_test.toy_id = toys.id
  JOIN
    cats AS cat_2 ON cattoy_test.cat_id = cat_2.id
  WHERE
    cqe != 'Jet'
  ORDER BY
    cats.name ASC
